...
func (p *GitHubCopilotOAuthProvider) Stream(ctx context.Context, req *provider.Request) (<-chan provider.StreamEvent, error) {
...
func (p *GitHubCopilotOAuthProvider) readSSE(body io.ReadCloser, ch chan<- provider.StreamEvent) {
	defer body.Close()
	defer close(ch)
...
func (p *GitHubCopilotOAuthProvider) buildBody(req *provider.Request) map[string]any {
...
func buildGitHubCopilotDynamicHeaders(messages []provider.Message) map[string]string {
	initiator := "user"
	if len(messages) > 0 {
