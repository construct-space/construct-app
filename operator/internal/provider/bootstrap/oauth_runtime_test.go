...
func TestProviderFromOAuthCredentialsSupportedProviders(t *testing.T) {
	...
	anthropicProv := FromOAuthCredentials("anthropic", creds)
	...
	codexProv := FromOAuthCredentials("openai-codex", creds)
	if codexProv == nil {
	...
	githubProv := FromOAuthCredentials("github-copilot", githubCreds)
	if githubProv == nil {
	...
	geminiProv := FromOAuthCredentials("google-gemini-cli", geminiCreds)
	if geminiProv == nil {
...
func TestOAuthConnectedProviderEntriesMarksStoredAndRuntimeProviders(t *testing.T) {
	...
	entries := ConnectedProviderEntries(authData, runnerProviders)
