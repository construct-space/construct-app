...
func RuntimeProviderID(id string) string {
...
func FromOAuthCredentials(providerID string, creds *oauth.Credentials) provider.Provider {
	if creds == nil {
		return nil
	}
	...
	switch providerID {
	case "anthropic":
		return connectors.NewAnthropicOAuth(connectors.OAuthConfig{
			ClientID:     connectors.OpenCodeClientID,
	...
	case "openai-codex":
		...
		return connectors.NewCodexOAuth(connectors.CodexOAuthConfig{
			AccessToken:  creds.Access,
			RefreshToken: creds.Refresh,
			AccountID:    accountID,
			Models:       desc.Models,
	...
	case "github-copilot":
		...
		return connectors.NewGitHubCopilotOAuth(connectors.GitHubCopilotOAuthConfig{
			AccessToken:      creds.Access,
			RefreshToken:     creds.Refresh,
	...
	case "google-gemini-cli":
		...
		return connectors.NewGoogleGeminiCLIOAuth(connectors.GoogleGeminiCLIOAuthConfig{
			AccessToken:  creds.Access,
			RefreshToken: creds.Refresh,
			ProjectID:    projectID,
			ExpiresAt:    time.UnixMilli(creds.Expires),
...
func appendOAuthRuntimeProviders(providers *[]provider.Provider, existing map[string]bool, authData oauth.StorageData) {
	for providerID, cred := range authData {
		...
		runtimeID := RuntimeProviderID(providerID)
		if runtimeID == "" || existing[runtimeID] {
			continue
		}
		if prov := FromOAuthCredentials(providerID, cred.Credentials); prov != nil {
...
func ConnectedProviderEntries(authData oauth.StorageData, runnerProviders []map[string]any) []map[string]any {
