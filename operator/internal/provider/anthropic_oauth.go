...
func (p *AnthropicOAuthProvider) Complete(ctx context.Context, req *Request) (*Response, error) {
...
func (p *AnthropicOAuthProvider) Stream(ctx context.Context, req *Request) (<-chan StreamEvent, error) {
	token, err := p.getToken()
	if err != nil {
		return nil, fmt.Errorf("auth: %w", err)
	}
...
func (p *AnthropicOAuthProvider) buildBody(req *Request, stream bool) map[string]any {
		...
		if m.Role == "tool" && m.ToolResult != nil {
			...
			for _, tc := range m.ToolCalls {
				...
				content = append(content, map[string]any{
					"type":  "tool_use",
					"id":    tc.ID,
					"name":  SanitizeToolName(mcpToolPrefix + tc.Name),
					"input": inputObj,
				})
			}
			msg["content"] = content
	...
	if len(req.Tools) > 0 {
		...
		for i, t := range req.Tools {
			tools[i] = map[string]any{
				"name":         SanitizeToolName(mcpToolPrefix + t.Name),
...
func (p *AnthropicOAuthProvider) parseResponse(data []byte) (*Response, error) {
	var raw struct {
		Content []struct {
			Type  string          `json:"type"`
...
func (p *AnthropicOAuthProvider) readSSE(body io.ReadCloser, ch chan<- StreamEvent) {
	defer body.Close()
	defer close(ch)
		...
		case "content_block_start":
			if cb, ok := event["content_block"].(map[string]any); ok {
				if bt, _ := cb["type"].(string); bt == "tool_use" {
					...
					ch <- StreamEvent{Type: "tool_call_start", ToolCall: currentToolCall}
				}
			}
