...
func (p *GoogleGeminiCLIOAuthProvider) Complete(ctx context.Context, req *Request) (*Response, error) {
...
func (p *GoogleGeminiCLIOAuthProvider) Stream(ctx context.Context, req *Request) (<-chan StreamEvent, error) {
...
func (p *GoogleGeminiCLIOAuthProvider) buildBody(req *Request) map[string]any {
	...
	if len(req.Tools) > 0 {
		...
		for i, toolDef := range req.Tools {
			functionDeclarations[i] = map[string]any{
				...
				"parametersJsonSchema": SanitizeToolSchema(toolDef.InputSchema),
			}
		}
...
func (p *GoogleGeminiCLIOAuthProvider) readSSE(body io.ReadCloser, ch chan<- StreamEvent) {
	defer body.Close()
	defer close(ch)
	...
	for scanner.Scan() {
		...
		for _, part := range candidate.Content.Parts {
			if part.Text != "" {
				...
				ch <- StreamEvent{Type: "text_delta", Text: part.Text}
			}
			if part.FunctionCall != nil {
				...
				ch <- StreamEvent{Type: "tool_call_start", ToolCall: &call}
			}
		}
...
func googleToolNameIndex(messages []Message) map[string]string {
