...
func (p *OpenAICompatProvider) readSSE(body io.ReadCloser, ch chan<- StreamEvent) {
	defer body.Close()
	defer close(ch)

	var textContent string
	var reasoningContent string
	toolCalls := map[int]*ToolCall{} // index → tool call (OpenAI sends by index)
...
func (p *OpenAICompatProvider) buildBody(req *Request) map[string]any {
...
func (p *OpenAICompatProvider) parseResponse(data []byte) (*Response, error) {
	var raw struct {
		Choices []struct {
			Message struct {
				Content          string `json:"content"`
