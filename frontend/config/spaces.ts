...
/**
 * Whether a real space theme has been registered for this id. Lets callers
 * distinguish actual spaces from host pseudo-routes (spaces, marketplace, …),
 * which getSpace() would otherwise answer with a generic box default.
 */
export function hasSpaceTheme(name: string): boolean {
  return spaceThemes.has(name)
}
