...
export const useBilling = () => {
  const api = useApi(appConfig.billingUrl)
  ...
  const fetchPlans = async () => {
    ...
    error.value = null
    try {
      const response = await api.get<BillingPlan[] | { plans: BillingPlan[] }>('/plans')
  ...
  const fetchSubscription = async () => {
    ...
    error.value = null
    try {
      const response = await api.get<BillingSubscription | { subscription: BillingSubscription | null }>('/subscription')
      const sub = response && 'subscription' in response ? response.subscription : response
  ...
  const createCheckout = async (planSlug: string, successUrl: string, cancelUrl: string): Promise<string> => {
    ...
    error.value = null
    try {
      const response = await api.post<{ url: string }>('/checkout', {
        plan_slug: planSlug,
  ...
  const createCreditCheckout = async (productId: string, successUrl: string, cancelUrl: string): Promise<string> => {
    loading.value = true
    error.value = null
    try {
      const response = await api.post<{ url?: string; checkout_url?: string }>('/credits/purchase', {
        product_id: productId,
        success_url: successUrl,
        cancel_url: cancelUrl,
      })
      const url = response.url || response.checkout_url
      if (!url) throw new Error('Checkout URL missing')
      return url
    } catch (err) {
      error.value = err instanceof Error ? err.message : 'Failed to create credit checkout'
      throw err
    } finally {
      loading.value = false
    }
  ...
  const createPortalSession = async (returnUrl: string): Promise<string> => {
    ...
    error.value = null
    try {
      const response = await api.post<{ url: string }>('/portal', {
  ...
  const fetchInvoices = async () => {
    ...
    error.value = null
    try {
      const response = await api.get<BillingInvoice[] | { invoices: BillingInvoice[] }>('/invoices')
      const items = Array.isArray(response) ? response : response.invoices
