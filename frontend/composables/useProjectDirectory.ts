...
// Module-level guard: ensure the local_path watcher is only installed once
...
export interface ProjectConfig {
  ...
  // "project"       → general dev, opens in Builder
  // "space-project" → Construct Space, opens in Space Developer
  // undefined       → legacy entries (treated as "project" for routing)
...
export function useProjectDirectory() {
  ...
  const getProjectsRoot = async (): Promise<string> => {
    ...
    // Check profile-scoped storage for saved preference
    const savedRoot = profileStorage.getItem(PROJECTS_ROOT_KEY)
    if (savedRoot) {
  ...
  const setProjectsRoot = async (path: string): Promise<boolean> => {
    ...
    try {
      ...
      await tauriFs.mkdir(normalizedPath, { recursive: true })

      state.projectsRoot = normalizedPath
      profileStorage.setItem(PROJECTS_ROOT_KEY, normalizedPath)

      // Also sync to operator via settings so hooks/agents use the right root
      try {
        const { useBrain } = await import('@/brain')
        const brain = useBrain()
        await brain.request('settings.set', { key: 'construct_projects_root', value: normalizedPath })
  ...
  const createProjectStructure = async (
    ...
    try {
      ...
      // Normalize separators for cross-platform safety (Windows uses \, macOS/Linux use /).
      // The configured root from settings is the trusted boundary.
      const configuredRoot = normalizePath(state.projectsRoot)
      if (!configuredRoot) {
        console.warn('[ProjectDirectory] No projects root configured in Settings. Set a projects folder in Settings → Projects to enable path confinement.')
      } else {
        const sep = configuredRoot.includes('\\') ? '\\' : '/'
        const normalizedProject = projectPath.replace(/[\\/]/g, sep)
        const normalizedRoot = configuredRoot.replace(/[\\/]/g, sep)
        const rootWithSep = normalizedRoot.endsWith(sep) ? normalizedRoot : `${normalizedRoot}${sep}`
        if (normalizedProject !== normalizedRoot && !normalizedProject.startsWith(rootWithSep)) {
          console.error(`Project path "${projectPath}" is outside the configured projects root "${configuredRoot}"`)
          return null
        }
      }

      // Create project directory + docs + .construct (for the manifest).
      // .construct/project.json is the marker the scanner uses to auto-
      // import this folder as a Construct project on next reload.
      const allDirs = [
        projectPath,
        `${projectPath}/docs`,
      ...
      await initTauri()
      if (!tauriFs) throw new Error('Tauri FS API not available')
      for (const dir of allDirs) {
        await tauriFs.mkdir(dir, { recursive: true })
      }

      // Write the manifest so `kind` (and the other standard fields) survive
      // a restart. Matches the org-project manifest shape.
      ...
      await tauriFs.writeTextFile(
        `${projectPath}/.construct/project.json`,
  ...
  const loadProjectConfig = async (projectPath: string): Promise<ProjectConfig | null> => {
    ...
    try {
      ...
      const kind: ProjectConfig['kind'] = rawKind === 'space-project' || rawKind === 'project' ? rawKind : undefined
  ...
  const ensureProjectStructure = async (
    ...
    try {
      await initTauri()
      if (!tauriFs) {
        console.error('[ensureProjectStructure] Tauri FS API not available')
        return false
      }
      ...
      let dirExists = false
      try {
        const entries = await tauriFs.readDir(targetPath)
      ...
      if (dirExists) {
        ...
        await tauriFs.mkdir(`${targetPath}/.construct`, { recursive: true })
        return true
      }
  ...
  // Guard: only install once across all consumers of this composable
  if (typeof window !== 'undefined' && projectStore && !_watcherInstalled) {
