...
// Don't auto-bounce when the user got here via "Add account" — they
// already have a live session for another profile and are intentionally
// signing up a new identity. The form stays mounted until registration
// completes, which switches the active profile and then routes onward.
watchEffect(() => {
  if (authStore.isAuthenticated && route.query.add !== '1') {
    router.push('/app')
  }
})
</script>
...
<template>
  <div class="min-h-screen bg-[var(--app-background)]">
    <div class="h-screen overflow-y-auto flex items-center justify-center px-6 py-10">
      <div class="w-full max-w-sm">
        ...
        <div class="flex flex-col items-center gap-4 mb-8">
          ...
          <p class="text-2xl text-center">
            <span class="text-[var(--app-muted)]">CONSTRUCT:</span><span class="font-bold text-[var(--app-foreground)]">REGISTER</span>
          </p>
        ...
        <Card variant="muted">
          <form class="space-y-4" @submit.prevent="handleSubmit">
            ...
            <RouterLink
              :to="route.query.add === '1' ? '/login?add=1' : '/login'"
              class="flex items-center justify-center gap-2 text-[var(--app-muted)] hover:text-app-accent transition-colors pt-1"
            >
