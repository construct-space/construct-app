...
// Paths under the projects root that the user has explicitly removed via
// "Remove From Construct". Prevents them from being re-added on next scan.
const STORAGE_KEY_HIDDEN = 'construct_hidden_project_paths'
const MAX_RECENTS = 20

// All spaces available to every project by default
const DEFAULT_SPACES: SpaceType[] = [
  'builder', 'space-developer', 'editor',
]
...
export const useProjectStore = defineStore('project', {
  ...
  actions: {
    async initialize() {
      ...
      this.projectsRoot = normalizePath(profileStorage.getItem(STORAGE_KEY_ROOT) || '')

      const rawExternals = JSON.parse(profileStorage.getItem(STORAGE_KEY_EXTERNALS) || '[]') as string[]
      this.externalPaths = [...new Set(rawExternals.map(path => normalizePath(path)).filter(Boolean))]
      profileStorage.setItem(STORAGE_KEY_EXTERNALS, JSON.stringify(this.externalPaths))

      const rawHidden = JSON.parse(profileStorage.getItem(STORAGE_KEY_HIDDEN) || '[]') as string[]
      this.hiddenPaths = [...new Set(rawHidden.map(path => normalizePath(path)).filter(Boolean))]
      profileStorage.setItem(STORAGE_KEY_HIDDEN, JSON.stringify(this.hiddenPaths))

      const rawRecents = JSON.parse(profileStorage.getItem(STORAGE_KEY_RECENTS) || '[]') as LocalProject[]
      ...
      profileStorage.setItem(STORAGE_KEY_RECENTS, JSON.stringify(this.recentProjects))
    },
      ...
      try {
        ...
        if (!this.projectsRoot) {
          ...
          if (defaultRoot) {
            ...
            profileStorage.setItem('construct_projects_root', defaultRoot)
          }
        } else {
        ...
        if (this.projectsRoot) {
          ...
          const hiddenSet = new Set(this.hiddenPaths)

          for (const entry of entries) {
            const config = entry.config
            // Only auto-import folders that were explicitly initialized as
            // Construct projects (have a `.construct/project.json`). This
            // prevents arbitrary sibling folders under the projects root
            // from being treated as projects.
            if (!config) continue
            // Skip empty directories named after spaces (stale root-level space folders)
            if (spaceNames.has(entry.name as SpaceType)) continue
            // Skip projects the user explicitly removed — persisted across restarts.
            if (hiddenSet.has(normalizePath(entry.path))) continue
        ...
        if (validExternals.length !== normalizedExternals.length) {
          ...
          profileStorage.setItem(STORAGE_KEY_EXTERNALS, JSON.stringify(validExternals))
        }

        // Fetch org projects — only when the user is BOTH org and
        // developer. Simply setting up an organization does not surface
        // org projects (mirrors useDeveloperGate's org gate).
        try {
          const { useOrgStore } = await import('@/stores/org')
          const orgStore = useOrgStore()
          const { useDeveloperGate } = await import('@/composables/useDeveloperGate')
          const developerGate = useDeveloperGate()
          if (orgStore.isEnabled && developerGate.isOrg.value) {
            const { useSource } = await import('@/composables/useSource')
            const api = useSource()
            const orgProjects = await api.get<Array<{
              id: string
              name: string
              description: string
              repo_url: string
            ...
            if (Array.isArray(orgProjects)) {
              for (const op of orgProjects) {
                const existing = projects.find(p => p.name === op.name)
                if (!existing) {
                  projects.push({
                    id: `org-${op.id}`,
                    name: op.name,
                    ...
                    is_external: false,
                    origin: 'org',
                    is_org: true, // deprecated alias; mirrored for older readers
                    org_project_id: op.id,
                  ...
                  // Enrich local project with org metadata
                  existing.origin = 'org'
                  existing.is_org = true // deprecated alias
                  existing.org_project_id = op.id
                  existing.repo_url = op.repo_url
                }
              }
            }
          }
        } catch { /* org store not available */ }
    ...
    async createProject(data: { name: string; description?: string; color?: string; spaces?: SpaceType[]; localPath?: string; kind?: 'project' | 'space-project' }) {
      this.loading = true
      this.error = null
      ...
      try {
        ...
        if (!this.projectsRoot) {
          ...
          if (root) {
            ...
            profileStorage.setItem(STORAGE_KEY_ROOT, this.projectsRoot)
        ...
        if (data.localPath) {
          ...
          createdPath = await projectDir.createProjectStructure(data.name, data.localPath, spaces, true, data.kind, data.description)
        } else {
          // Derive path from projectsRoot/name
          createdPath = await projectDir.createProjectStructure(data.name, this.projectsRoot, spaces, false, data.kind, data.description)
        }
        ...
        // If the user had this path hidden previously (e.g. removed then
        // recreated at the same location), unhide it so it shows up.
        if (this.hiddenPaths.includes(project.path)) {
          this.unhideProject(project.path)
        }
    ...
    removeProject(path: string) {
      ...
      // Persist removal so the next scan of projectsRoot doesn't re-add it.
      // Only meaningful when the path is under the managed root; external
      // paths were already dropped above and won't reappear from a scan.
      if (!this.hiddenPaths.includes(normalizedPath)) {
        this.hiddenPaths = [...this.hiddenPaths, normalizedPath]
        profileStorage.setItem(STORAGE_KEY_HIDDEN, JSON.stringify(this.hiddenPaths))
      }
    ...
    /** Re-import a previously removed project (removes from hidden list). */
    unhideProject(path: string) {
      const normalizedPath = normalizePath(path)
      if (!this.hiddenPaths.includes(normalizedPath)) return
      this.hiddenPaths = this.hiddenPaths.filter(p => p !== normalizedPath)
      profileStorage.setItem(STORAGE_KEY_HIDDEN, JSON.stringify(this.hiddenPaths))
    },
    ...
    async deleteProjectFromDisk(path: string) {
      ...
      try {
        ...
        if (await tauriFs.exists(path)) {
          // NOTE: This is a permanent delete, not move-to-trash.
          // Tauri does not provide a cross-platform trash API.
