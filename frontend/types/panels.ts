...
export type SpaceType = 'code' | 'ui' | 'git' | 'ai' | 'notes' | 'builder' | 'kanban' | 'terminal' | 'chat' | 'design' | 'docs' | 'calendar'
...
export const DEFAULT_LAYOUTS: Record<SpaceType, PanelLayout> = {
  ...
  builder: {
    panels: [
      { id: 'chat-1', panelType: 'builder-chat', position: { col: 1, row: 1 }, size: { cols: 2, rows: 3 } },
      { id: 'files-1', panelType: 'file-explorer', position: { col: 3, row: 1 }, size: { cols: 1, rows: 3 } }
    ],
    gridCols: 3,
    gridRows: 3
  },
