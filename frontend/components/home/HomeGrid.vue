...
function getCellSize() {
  ...
  return { cellW: rect.width / props.gridCols, cellH: rect.height / props.gridRows }
}
...
function screenToCell(clientX: number, clientY: number): { col: number; row: number } | null {
  ...
  const row = Math.floor((clientY - rect.top) / (rect.height / props.gridRows))
  if (col < 0 || col >= props.gridCols || row < 0 || row >= props.gridRows) return null
  return { col, row }
}

// Build occupancy grid (which cells are taken)
function buildOccupancy(excludeId?: string): boolean[][] {
  const grid = Array.from({ length: props.gridRows }, () => Array(props.gridCols).fill(false))
  for (const item of props.items) {
    if (excludeId && item.instanceId === excludeId) continue
    for (let r = item.y; r < item.y + item.h; r++) {
      for (let c = item.x; c < item.x + item.w; c++) {
        if (r < props.gridRows && c < props.gridCols) grid[r][c] = true
      }
    }
  }
  return grid
...
function fitsAt(x: number, y: number, w: number, h: number, excludeId: string): boolean {
  if (x < 0 || y < 0 || x + w > props.gridCols || y + h > props.gridRows) return false
...
function onResizeStart(e: MouseEvent, item: WidgetPlacement) {
  ...
  const onMove = (ev: MouseEvent) => {
    ...
    const targetH = Math.max(1, Math.min(props.gridRows - item.y, Math.round(startH + (ev.clientY - startY) / cellH)))
