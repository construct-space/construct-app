...
<template>
  <nav class="flex items-center gap-1.5 text-sm ">
    <!-- Space icon -->
    <div v-if="spaceConfig?.icon" class="size-5 rounded flex items-center justify-center shrink-0"
    ...
    <template v-for="(crumb, idx) in displayBreadcrumbs" :key="idx">
      ...
      <button v-if="crumb.to" class="hover:underline transition-colors" style="color: var(--app-muted)"
        @click="router.push(crumb.to)">
        {{ crumb.label }}
      </button>

      <span v-else class="font-medium" style="color: var(--app-foreground)">
        {{ crumb.label }}
      </span>
    </template>
  </nav>
