...
/**
 * GoogleCard — Gemini via API key (AI Studio) or Cloud Code Assist OAuth.
 * Mirrors AnthropicCard: api_key slot + monthly (OAuth) slot.
 * OAuth flow id `google-gemini-cli` is already wired in the operator;
 * a future Antigravity mode would slot in as a second OAuth provider.
 */
...
const OAUTH_PROVIDER_ID = 'google-gemini-cli'
...
function connectorId(): string {
  return active.value === 'monthly' ? 'google-gemini-cli' : 'google'
}
...
const headerStatus = computed(() => {
  ...
  return active.value === 'monthly' ? 'Google signed in' : 'API key configured'
})
</script>
  ...
  <Card class="mb-4">
      ...
      <div class="flex w-full items-start justify-between gap-3">
        <div class="flex min-w-0 items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="text-sm text-[var(--app-muted)] mt-0.5">Gemini 2.5 Pro/Flash · vision · tools · 1M context</p>
          </div>
        </div>
      ...
      >
        ...
        <div v-if="!card.isConfigured(provider.id) || editing" class="flex gap-2 items-center" @click.stop>
          <div class="flex-1 relative">
            <Input v-model="draft" :type="visible ? 'text' : 'password'" placeholder="AIza…" size="sm" />
        ...
        <p v-if="!card.isConfigured(provider.id)" class="text-[10px] text-[var(--app-muted)] mt-2">
          Get a key at <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener" class="text-[var(--app-accent)] hover:underline" @click.stop>aistudio.google.com</a>
        </p>
      </Card>
      ...
      >
          ...
          <div class="flex w-full items-start justify-between gap-3">
            <div class="flex items-start gap-2 min-w-0">
              ...
              <div class="flex flex-col gap-1 min-w-0">
                <h4 class="text-xs tracking-[0.08em] uppercase font-normal text-[var(--app-foreground)] leading-tight after:content-['.'] after:text-[var(--app-accent)]">Sign in with Google</h4>
        ...
        <div v-else-if="!oauthState.connected" @click.stop>
          <p class="text-xs text-[var(--app-muted)] mb-2">
            Use your Google account via Cloud Code Assist. Opens a browser window; no API billing.
          </p>
          <div class="flex items-center gap-2">
            <Button size="sm" :loading="oauth.loading.value[OAUTH_PROVIDER_ID]" label="Sign in with Google" @click="signIn" />
            <Button
    ...
    <template v-if="!collapsed" #footer>
      <div v-if="!isAuthed" class="text-xs text-[var(--app-muted)]">
        {{ active === 'monthly' ? 'Sign in with Google to see available models.' : 'Add an API key to see available models.' }}
      </div>
