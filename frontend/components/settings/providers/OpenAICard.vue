...
/**
 * OpenAICard — GPT via API key or ChatGPT Plus OAuth.
 */

import { computed, onMounted, ref, watch } from 'vue'
import { Alert, Badge, Button, Card, Input } from '@construct-space/ui'
...
type OpenAIModel = AIProvider['models'][number] & {
  available_on_api_key?: boolean
  available_on_monthly?: boolean
}
const apiOnlyModels = computed(() =>
  ((props.provider.models || []) as OpenAIModel[]).filter((m) =>
    m.available_on_api_key !== false && m.available_on_monthly === false,
  ),
)
...
function connectorId(): string {
  return active.value === 'monthly' ? 'openai-oauth' : 'openai'
}
...
const headerStatus = computed(() => {
  ...
  return active.value === 'monthly' ? 'ChatGPT signed in' : 'API key configured'
})
</script>
...
<template>
  <Card class="mb-4">
      ...
      <div class="flex w-full items-start justify-between gap-3">
        <div class="flex min-w-0 items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="text-sm text-[var(--app-muted)] mt-0.5">GPT-5.x + o-series · reasoning · tools</p>
          </div>
        </div>
      ...
      >
        ...
        <div v-if="!card.isConfigured(provider.id) || editing" class="flex gap-2 items-center" @click.stop>
          <div class="flex-1 relative">
            <Input v-model="draft" :type="visible ? 'text' : 'password'" placeholder="sk-…" size="sm" />
        ...
        <p v-if="!card.isConfigured(provider.id)" class="text-[10px] text-[var(--app-muted)] mt-2">
          Get a key at <a href="https://platform.openai.com" target="_blank" rel="noopener" class="text-[var(--app-accent)] hover:underline" @click.stop>platform.openai.com</a>
        </p>
      </Card>
      ...
      >
          ...
          <div class="flex w-full items-start justify-between gap-3">
            <div class="flex items-start gap-2 min-w-0">
              ...
              <div class="flex flex-col gap-1 min-w-0">
                <h4 class="text-xs tracking-[0.08em] uppercase font-normal text-[var(--app-foreground)] leading-tight after:content-['.'] after:text-[var(--app-accent)]">ChatGPT Plus</h4>
        ...
        <div v-else-if="!oauthState.connected" @click.stop>
          <p class="text-xs text-[var(--app-muted)] mb-2">
            Use your ChatGPT Plus subscription. <strong class="text-amber-500">Smaller subset</strong> — codex and o3 stay API-only.
          </p>
          <div class="flex items-center gap-2">
            <Button size="sm" :loading="oauth.loading.value[OAUTH_PROVIDER_ID]" label="Sign in with ChatGPT" @click="signIn" />
            <Button
    ...
    <!-- Switch-warning -->
    <Alert
      v-if="!collapsed && active === 'api_key' && apiOnlyModels.length > 0"
      color="warning"
      class="mt-4"
      :description="`Switching to ChatGPT would drop access to ${apiOnlyModels.length} API-only model(s): ${apiOnlyModels.slice(0, 3).map((m) => m.id).join(', ')}${apiOnlyModels.length > 3 ? '…' : ''}`"
    />

    <!-- Models -->
    <template v-if="!collapsed" #footer>
      <div v-if="!isAuthed" class="text-xs text-[var(--app-muted)]">
        {{ active === 'monthly' ? 'Sign in with ChatGPT to see available models.' : 'Add an API key to see available models.' }}
      </div>
