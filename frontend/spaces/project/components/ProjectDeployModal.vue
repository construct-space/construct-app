...
const { state, deploy, generateAppName, loadDeployInfo, waitForUrlReady } = useBasepodDeploy()

// Whether this project was already deployed before (a prior .construct/
// deploy.json). Re-deploys reuse the existing SSL cert, so we skip the
// provisioning wait; first deploys must wait for the cert before opening.
const wasDeployed = ref(false)
// Set when the modal closes/unmounts so an in-flight readiness poll aborts.
let aborted = false
...
watch(() => props.project, async (p) => {
  ...
  if (!codePath) return
  // Has this project been deployed before? Determines whether we wait for
  // SSL provisioning after deploy.
  wasDeployed.value = !!(await loadDeployInfo(codePath))
  try {
...
async function handleDeploy() {
  ...
  if (hasUncommitted.value && deployMessage.value.trim()) {
    try {
      ...
      await invoke('run_shell_command', {
        command: 'git',
        args: ['commit', '-m', deployMessage.value.trim()],
        cwd: sourceDir,
      })
  ...
  // After a successful deploy, the URL is known but may not serve yet.
  if (state.value.url && !state.value.error) {
    if (wasDeployed.value) {
      // Re-deploy: cert already provisioned — open immediately.
      state.value.urlReady = true
    } else {
      // First deploy: wait for the SSL cert to provision (poll until 200),
      // then reveal "Open in Browser". Mark deployed so an in-session
      // re-deploy skips the wait.
      await waitForUrlReady(state.value.url, { signal: () => aborted })
      wasDeployed.value = true
    }
  }
}
        ...
        <div class="p-6">
          ...
          <div v-if="state.url" class="mb-5 p-4 rounded-lg border border-emerald-500/30 bg-emerald-500/5">
            <div class="flex items-center gap-2 mb-2">
              <Loader2 v-if="state.verifying" class="size-4 text-emerald-400 animate-spin" />
              <CheckCircle v-else class="size-4 text-emerald-400" />
              <p class="text-sm text-emerald-400 font-medium">
                {{ state.verifying ? 'Provisioning SSL certificate…' : 'Deployed!' }}
              </p>
            </div>
            <p v-if="state.verifying" class="text-xs text-emerald-400/70 mb-1">
              First deploy can take up to a minute while the certificate is issued.
            </p>
            <button
          ...
          <div class="flex justify-end gap-3">
            ...
            <button
              v-if="state.url"
              class="px-4 py-2 rounded-lg bg-emerald-500 text-white text-sm font-medium hover:bg-emerald-600 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              :disabled="state.verifying"
              @click="openUrl(state.url!)"
            >
              <Loader2 v-if="state.verifying" class="size-4 animate-spin" />
              <ExternalLink v-else class="size-4" />
              {{ state.verifying ? 'Waiting for SSL…' : 'Open in Browser' }}
            </button>
          </div>
        </div>
      </div>
