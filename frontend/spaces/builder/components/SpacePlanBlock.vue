...
interface PlanDoc {
  ...
  title: string
  /**
   * True when a `write` tool call for this path completed successfully
   * in the same assistant message. False/undefined means the envelope
   * claimed a doc that wasn't actually persisted to disk — we surface
   * that clearly so the user isn't misled.
   */
  persisted?: boolean
}
    ...
    <div v-if="safeDocs.length" class="mt-4">
      ...
      <div class="flex flex-col gap-1">
        <button
          ...
          :title="d.persisted ? 'Saved to disk' : 'Plan envelope referenced this file but no successful write call was recorded — it may not exist on disk'"
          @click="emit('open-doc', d)"
        >
          <FileText
            :size="12"
            :style="`color: ${d.persisted ? 'var(--app-muted)' : 'var(--app-warning, #d97706)'}; flex-shrink: 0; opacity: ${d.persisted ? 1 : 0.7}`"
          />
          <span
            :style="`color: ${d.persisted ? 'var(--app-accent)' : 'var(--app-muted)'}; ${d.persisted ? '' : 'text-decoration: line-through; opacity: 0.6;'}`"
            class="truncate"
          >{{ d.path }}</span>
          <span
          ...
          <span
            v-if="!d.persisted"
            class="ml-auto inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider"
            style="background: color-mix(in srgb, var(--app-warning, #d97706) 14%, transparent); color: var(--app-warning, #d97706)"
          >
            not saved
          </span>
        </button>
