...
import { useSkills, type SkillInfo } from '@/composables/useSkills'
import { useBuilderPreview } from '@/composables/useBuilderPreview'
import SkillsMenu from '../components/SkillsMenu.vue'
import TasksPanel from '@/spaces/space-developer/components/TasksPanel.vue'
...
import BlockRenderer, { type Block, type ToolCallBlock } from '../components/BlockRenderer.vue'
...
import { parseHandoff, handoffLabel, performHandoff, type HandoffTarget } from '@/lib/agentHandoff'
...
const { isLoading: skillsLoading, listSkills } = useSkills()
const preview = useBuilderPreview()

async function togglePreview() {
  if (preview.running.value || preview.starting.value) {
    await preview.stop()
    return
  }
  if (!projectPath.value) {
    errorMsg.value = 'Open a project first.'
    return
  }
  await preview.start(projectPath.value)
  if (preview.error.value) errorMsg.value = preview.error.value
}
...
// Default to 'plan' so new projects start in the planning workflow.
// Restored sessions (restoreSession) overwrite this with the saved mode.
const mode = ref<Mode>('plan')
const messages = ref<Message[]>([])
// Live task tracker — populated from task_create/task_update/task_list tool
// results in the stream. Rendered by TasksPanel in the sidebar.
...
// Session history lives IN the project: `<projectPath>/.construct/builder-history.json`.
...
// hiding in browser storage. Separate file from spacedev-history.json so the
// two agents' histories don't collide when both are used on the same project.
...
function historyPath(): string | null {
  ...
  if (!path) return null
  return `${path}/.construct/builder-history.json`
}
...
async function saveSession() {
  ...
  try {
    ...
    if (root) {
      ...
      if (!(await exists(constructDir))) await mkdir(constructDir, { recursive: true })
    }
...
async function restoreSession() {
  ...
  try {
    ...
    if (Array.isArray(parsed.messages)) {
      ...
      // A restored session (reload / open-in-new-window) is historical —
      // nothing is live. Finalize any tool block left "running" when it was
      // saved (otherwise it spins forever), clear the streaming flag, and
      // rebuild the task tracker from saved task_* results so the Tasks
      // panel survives the reload.
      tasks.value = new Map()
    ...
    if (parsed.mode === 'plan' || parsed.mode === 'code') mode.value = parsed.mode
  } catch { /* bad JSON or permissions — start fresh */ }
}
...
function stopRun() {
  if (cancelStream) {
    try { cancelStream() } catch { /* already stopped */ }
  }
  cancelStream = null
  sending.value = false
...
const DANGLING_PATTERNS: RegExp[] = [
  ...
  // "Creating tasks:", "Building the site…", "Setting up the project"
  /\b(creating|building|scaffolding|setting up|adding|implementing|refactoring|writing|generating) (the |a )?(tasks?|files?|project|site|page|app|components?|layout|landing)\b/i,
  /\bcontinuing\.{0,3}$/i,
  /\bone (moment|sec|second)\b/i,
  // Dangles on a lead-in colon ("…match this:", "Creating tasks:") — the
  // promised list/work never came because the turn ended.
  /[:：]\s*$/,
]
...
const canContinue = computed(() => {
  ...
  const text = last.content || (last.blocks ?? [])
...
// Auto-continue: when the model narrates intent then ends the turn without
// acting (or hits the token limit), the builder resumes on its own so the
// user doesn't have to click Continue every time. Capped per user turn to
// prevent runaway loops; the counter resets whenever the user sends a real
// message (see send()).
...
// Skills explicitly loaded by the agent via load_skill during this session.
// The operator emits a skill.loaded event each time load_skill succeeds.
const sessionLoadedSkills = ref<SkillInfo[]>([])

let messageCounter = 0
function nextId() { return `msg-${Date.now()}-${++messageCounter}` }
...
function modeDirective(): string {
  if (mode.value === 'plan') {
    ...
    return [
      ...
      '- `plan`: FINAL state, all decisions settled. **You MUST call the `write` tool BEFORE emitting the JSON envelope**: `write({path: "docs/plan-' + stamp + '.md", content: "<full plan markdown>"})`. The envelope\'s `docs` array then points to the file you just wrote. docs/ is the only write path allowed in PLAN. Plan content: classification, identity, pages, data model, user flows, files to create/edit, verification targets. Without the write call the plan is lost between turns — do NOT skip it.',
  ...
  return [
    'Mode: CODE — execute. Don\'t narrate. Don\'t announce. Do.',
    '',
    'Environment: you are the Builder agent running INSIDE the Construct desktop app, not a terminal. The user sees your chat, a file Explorer, and a Preview pane (an in-app browser) — they have NO terminal and cannot run commands. To run or show the project you have the `start_preview` tool: it starts the dev server / serves the build and opens it in the Preview pane (works for Vite/Next/static sites, Go, Python, Rust). `bash` is for one-shot build/verify steps whose output you read — never for long-running dev servers (they block the turn).',
    '',
    'Required workflow this turn, in this exact order:',
    '',
    '1. **Read the plan.** If `docs/plan-*.md` exists, `read` the newest one. If not, scan the prior assistant messages in this session for the architect.v1 plan envelope JSON and parse it. Either way, you must enter step 2 with a concrete list of work items.',
    '',
    '2. **Create tasks.** Call `task_create` once per discrete work item — one task per file to create, one per feature to wire, one per verification step. Aim for 5–15 tasks. Keep titles short and verb-led ("Scaffold index.html", "Wire Tailwind CDN", "Build Hero section", "Add Products grid", "Verify build", …). Use `task_list` once to confirm the plan is captured.',
    '',
    '3. **Execute them.** For each task, in order: `task_update` to `in_progress`, do the work (`write`, `edit`, `bash`, `read`), then `task_update` to `completed`. Move to the next task in the SAME response — do not end your turn between tasks. If a task is blocked, `task_update` to `error` with a one-line reason and continue with the next independent task.',
    '',
    '4. **Verify, then show it live.** Make the last 1–2 tasks verification: run the build (`bash`) and confirm it compiles. Then **call `start_preview`** to launch the project and open it in the Preview pane.',
    '5. **Look at it.** After `start_preview`, call `list_windows` to find the preview window\'s label, then `screenshot_window(window_label)` to capture it — the screenshot comes back to you as an image you can actually SEE. Confirm the page renders (not blank, no error overlay) and matches the request; if it\'s broken, fix it and re-screenshot. Then briefly state what you verified.',
    '',
    'Hard rules:',
    '- NEVER tell the user to run a command (`npm run preview`, `npm run dev`, `npm start`, `open …`, etc.) — they have no terminal. To show the result, CALL `start_preview` yourself. To re-run later, they click the Preview button in the header.',
    '- Never launch a dev/preview server with `bash` (it blocks the turn) — use `start_preview`.',
    '- Never end a turn after "Now I\'ll…" or "Building now…" or "Let me check…". Keep tool-calling.',
    '- A turn ends only when: every task is `completed` or `error`, OR you need an answer from the user, OR you hit a hard blocker you can name.',
    '- One `bash pwd && ls` at the start is fine. After that, stop re-orienting — read the plan and go.',
    '- Tailwind CDN / static sites: no `npm install`, no build step. Write the HTML, then `start_preview` to open it.',
  ].join('\n')
}
...
async function send(blocks: RequestBlock[], opts: { auto?: boolean } = {}) {
  ...
  // Steer mid-stream: if the user sends a new message while the agent
  // is still running, abort the current turn so the new one lands as
  // the next user turn in the same operator session. The session id is
  // preserved, so the agent sees the prior work + the steer correction
  // in context on the next turn.
  if (sending.value) {
    stopRun()
  ...
  // call send directly — there's no shared `input` ref anymore.
  const text = blocks
  ...
  const fileBlocks = blocks
    .filter((b): b is FileLike => b.type === 'file' && !!(b as FileLike).path)
    .map((b) => ({
      type: 'file' as const,
  ...
  // Include the attachment filenames in the displayed message so the
  // user can see what they sent. The actual bytes go to the model via
  // content_blocks below.
  ...
  // Images are inlined as data URLs; non-image files are appended to the
  // prompt as paths so the agent can `read` them.
  ...
  const onChunk = (chunk: BrainChunk) => {
    ...
    // Brain doesn't currently emit skill.loaded; the SkillsMenu stays empty during a brain run.
    if (chunk.type === 'skill.loaded') {
      const id = chunk.data?.id as string | undefined
      const name = chunk.data?.name as string | undefined
      const description = chunk.data?.description as string | undefined
      if (id && !sessionLoadedSkills.value.find(s => s.id === id)) {
        sessionLoadedSkills.value = [
          ...sessionLoadedSkills.value,
          { id, name: name ?? id, description: description ?? '', category: '', version: '', state: 'active', dependencies: [], hooksCount: 0, toolsCount: 0 } as SkillInfo,
        ]
      }
      return
    }
    ...
    if (chunk.type === 'tool_call') {
      // Unwrap the pi-shape call_tool dispatcher → the real tool + its args.
      const { tool, input } = unwrapToolCall(String(chunk.data?.name ?? ''), chunk.data?.input)
      // `||` not `??`: the brain sometimes sends an empty-string id, which must
      // still fall through to a unique fallback so blocks don't collide.
      const callId = String(chunk.data?.id || chunk.data?.call_id || `${tool}-${Date.now()}`)
      // The brain fires tool_call TWICE per call: once at tool_use_start with
      // empty input (so the card renders immediately as call_tool), then again
      // with the assembled args. Merge by callId — don't push a duplicate, and
      // don't let the empty first emission clobber the resolved tool/input.
      ...
      if (tool === 'start_preview' && projectPath.value) togglePreview()
      scrollToBottom()
      return
    }
    ...
    if (chunk.type === 'tool_result') {
      ...
      // The brain's tool_result is `{id, output, is_error}` — no tool name —
      // so the real tool is recovered from the correlated call block below.
      ...
      // Correlate to the matching call: by id; else the most recent running
      // block of the same tool (when the result carried a name); else the
      // oldest running block of any tool. The brain often sends an empty id
      // and never a name on results — without the FIFO fallback the call half
      // spins "running" forever and task results are dropped.
      const existing =
        msg.blocks.find((b): b is ToolCallBlock => b.type === 'tool' && b.callId === callId && !!callId)
      ...
      // event has none, so the task tracker / tree refresh need it from here.
      ...
      if (!isError && toolName && WRITE_TOOLS.has(toolName)) scheduleTreeRefresh()
      scrollToBottom()
      return
    }
  ...
  const onDone = (result: Record<string, unknown>) => {
    ...
    if (msg) {
      ...
      if (errText) {
        ...
        // The model sometimes ends a turn after announcing intent ("Now I'll
        // build the site.", "Creating tasks:") without doing the work, or hits
        // the token limit mid-task. Rather than make the user click Continue
        // every time, auto-resume (capped). Only when the turn did no work and
        // we have a session to continue — a turn that ended after real tool
        // calls is a normal stop.
        const stopReason = result?.stop_reason as string | undefined
        if (stopReason && stopReason !== 'tool_use') {
          const madeToolCalls = (msg.blocks ?? []).some(b => b.type === 'tool')
          const danglingNarration = stopReason === 'end_turn' && !madeToolCalls
            && mode.value !== 'plan' && endsOnDanglingIntent(msg.content || '')
          const hitTokenLimit = stopReason === 'max_tokens'
          const canAuto = (danglingNarration || hitTokenLimit)
          ...
          if (canAuto) {
            ...
            footer = '\n\n_Stopped on a stop sequence._'
          } else if (stopReason !== 'end_turn') {
            footer = `\n\n_Stopped: ${stopReason}._`
          }
    ...
    saveSession()
    scrollToBottom()
    // Resume on the next tick, after sending.value has settled to false.
    if (autoFire) {
    ...
    cancelStream = await session.brain.prompt(
      {
        prompt: task,
        agent_id: AGENT_ID,
        ...(images.length ? { content: [{ type: 'text' as const, text: task }, ...images] } : {}),
        ...(modelId ? { model: modelId } : {}),
        ...(lastSessionId.value ? { session_id: lastSessionId.value } : {}),
        ...(projectPath.value ? { project_dir: projectPath.value } : {}),
      },
...
function coerceArchitectEnvelope(parsed: Record<string, unknown>): ArchitectEnvelope | null {
  ...
  if (parsed.state === 'questions'
    ...
    && typeof (parsed.questions[0] as Record<string, unknown>)?.question === 'string') {
...
function tryPromoteToPlanBlock(msg: Message) {
  ...
  const textFromBlocks = (msg.blocks ?? [])
    ...
    .map(b => b.text)
    .join('\n')
  const source = [textFromBlocks, msg.content].filter(Boolean).join('\n').trim()
  if (!source) return
  ...
  if (envelope.state === 'plan') {
    // Compute which docs were actually persisted: scan this message's
    // tool blocks for a successful `write` call whose input.path matches
    // a doc.path. Anything not matched is just the envelope claim — we
    // shouldn't show "Saved to" for files that don't exist on disk.
    const writtenPaths = new Set<string>()
    for (const b of (msg.blocks ?? [])) {
      if (b.type !== 'tool') continue
      if (b.tool !== 'write') continue
      if (b.state !== 'done') continue
      try {
        const inp = b.input ? JSON.parse(b.input) as { path?: string } : null
        if (inp?.path) writtenPaths.add(inp.path)
      } catch { /* malformed input, skip */ }
    }
    const docs = (envelope.docs ?? []).map(d => ({
      ...d,
      persisted: writtenPaths.has(d.path),
    }))
    promoted = {
      type: 'plan',
...
// If the agent detected a Construct Space request and emitted a handoff
// marker, strip it from the last text block and append a handoff card the
// user can click to switch spaces.
function tryPromoteToHandoffBlock(msg: Message) {
  ...
  // Overwrite the text blocks with the cleaned text (without marker) and
  // append a handoff card. Idempotent: strip any existing handoff block
  // first so repeated calls don't stack cards.
  const kept: Block[] = (msg.blocks ?? []).filter(b => b.type !== 'handoff')

  // Replace text blocks with cleaned content.
  const withoutText: Block[] = kept.filter(b => b.type !== 'text')
  if (parsed.cleanText.trim()) {
    withoutText.push({ type: 'text', text: parsed.cleanText })
  }
  withoutText.push({
    type: 'handoff',
...
async function onPlanAction(action: { id: string; label: string }) {
  ...
  if (id.includes('code') || id === 'build' || id === 'proceed' || id === 'execute') {
    mode.value = 'code'
    await send([{ type: 'text', content: 'Proceed with the plan you just produced. Scaffold, implement, and verify.' }])
    return
  }
  // Fallback: treat unknown actions as prompts the user can send manually.
  // Nowhere to stick the pre-filled text now that AgentInput owns its own
  // state — firing send directly is the cleanest equivalent behavior.
...
function onPlanOpenDoc(doc: { path: string; title: string }) {
  ...
  const abs = projectPath.value ? `${projectPath.value}/${doc.path}` : doc.path
  errorMsg.value = `Saved plan at ${abs}`
  setTimeout(() => { if (errorMsg.value?.startsWith('Saved plan')) errorMsg.value = null }, 4000)
}
...
async function onQuestionAnswer(questionId: string, answer: string | string[]) {
  ...
  await send([{ type: 'text', content: `Answer for ${questionId}: ${text}` }])
}
...
async function handleCreateProject(name: string, description?: string) {
  ...
  try {
    ...
    if (result.success && result.data) {
      // Builder projects are regular directories — no `kind: 'space-project'`
      // marker. That field was spacedev-specific (telling consumers the
      // project contains `space-*/` subdirs). Builder works on anything, so
      // we leave the project unmarked.
      showCreateModal.value = false
      const key = getProjectRouteKey(result.data)
      router.replace(`/app/projects/${encodeURIComponent(key)}/builder`)
    } else {
...
onMounted(async () => {
  ...
  // Previously restoreSession fired-and-forgot, which left the chat
  // scrolled to the top on page open even with persisted history.
  ...
  // Handoff seed: if we were navigated here with ?q=<prompt> (from another
  // space's "Switch to Builder" button), auto-send so the user doesn't
  // retype. Strip the param first so reloads don't resend.
...
onUnmounted(() => {
  ...
  // Kill any running preview dev server so we don't leak the child
  // process when the user navigates away. No-op if nothing is running.
  preview.stop()
})
</script>
...
<template>
  ...
  >
    <ToolbarSlot name="left">
      <SkillsMenu :skills="sessionLoadedSkills" :loading="skillsLoading" />
    </ToolbarSlot>
    ...
    <ToolbarSlot name="right">
      <div class="flex items-center gap-2">
        ...
        <button
          v-if="hasProject"
          type="button"
          class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-semibold tracking-wider transition cursor-pointer hover:brightness-125 disabled:cursor-not-allowed disabled:opacity-60"
          :style="preview.running.value
            ? 'background: rgba(239, 68, 68, 0.12); color: rgb(239, 68, 68); border: 1px solid rgba(239, 68, 68, 0.3)'
            : 'background: var(--app-surface); color: var(--app-foreground); border: 1px solid var(--app-border)'"
          :disabled="preview.starting.value"
          :title="preview.running.value
            ? (preview.url.value ? `Running at ${preview.url.value} — click to stop` : 'Running — click to stop')
            : 'Start dev server and open preview in the in-app browser'"
          @click="togglePreview"
        >
          <Loader2 v-if="preview.starting.value" :size="11" class="animate-spin" />
          <Square v-else-if="preview.running.value" :size="11" />
          <Play v-else :size="11" />
          {{ preview.running.value ? 'STOP' : 'PREVIEW' }}
        </button>
        <div
          class="inline-flex rounded-md overflow-hidden text-[11px] font-semibold tracking-wider"
          style="border: 1px solid var(--app-border)"
        >
          <button
            type="button"
            class="px-3 py-1 transition cursor-pointer"
            :class="mode === 'plan' ? '' : 'hover:bg-white/5 hover:text-[var(--app-foreground)]'"
            :style="mode === 'plan'
          ...
          <button
            type="button"
            class="px-3 py-1 transition cursor-pointer"
            :class="mode === 'code' ? '' : 'hover:bg-white/5 hover:text-[var(--app-foreground)]'"
            :style="mode === 'code'
              ? 'background: var(--app-accent); color: var(--app-accent-foreground)'
              : 'background: transparent; color: var(--app-muted)'"
            @click="mode = 'code'"
    ...
    <!-- TODO: re-wire to brain telemetry (matched skills not emitted yet) -->
    <div v-if="false" class="flex items-center justify-center gap-1.5 py-1 shrink-0">
    ...
    <div class="flex-1 flex min-h-0">
      <div class="flex-1 flex flex-col min-w-0">
        ...
        >
          ...
          >
            ...
            <div class="max-w-md">
              ...
              >
                Describe what you want. I'll plan, implement, and verify it — landing pages, sites, apps, anything.
              </p>
            </div>
            <button
              v-if="!hasProject"
          ...
          >
            ...
            >
              <BlockRenderer
                :blocks="msg.blocks ?? (msg.content ? [{ type: 'text', text: msg.content }] : [])"
                :streaming="!!msg.streaming"
                :status-text="msg.streaming ? liveStatusText : ''"
                :hide-json-text="mode === 'plan'"
                @plan-action="onPlanAction"
                @plan-open-doc="onPlanOpenDoc"
                @question-answer="onQuestionAnswer"
          ...
          <div
            ...
            style="background: rgba(239, 68, 68, 0.08); color: rgb(239, 68, 68); border: 1px solid rgba(239, 68, 68, 0.25)"
          >
            {{ errorMsg }}
          </div>
        </div>
        ...
        <!--
          Wrapper gives the dock visual breathing room above RunControls.
          AgentInput's own .dock-input class sets mx-3 mb-3, so we only
          need to push it down from the IDLE row here — otherwise the
          input sits flush against the status bar.
        -->
        <div v-else class="pt-2" style="background: var(--app-background)">
          <AgentInput
            variant="editorial"
            :loading="sending"
            :placeholder="mode === 'plan' ? 'Describe what you want. I\'ll plan only…' : 'Tell me what to build…'"
            @send="send"
            @stop="stopRun"
          />
        </div>
      ...
      <aside
        class="w-64 shrink-0 border-l flex flex-col min-h-0"
        style="border-color: var(--app-border); background: var(--app-surface)"
      >
        <!-- Live task tracker — populated from task_* tool results in the
             stream. Capped + scrollable so it never crowds out the Explorer. -->
        <div
          v-if="tasks.size > 0"
        ...
        <div
          class="px-3 py-2 text-xs font-semibold flex items-center gap-2 border-b"
        ...
        >
          ...
          <button
            type="button"
            class="p-1 rounded transition disabled:opacity-40"
            style="color: var(--app-muted)"
            :disabled="!projectPath || treeLoading"
            title="Refresh"
        ...
        <div class="flex-1 overflow-y-auto p-2 text-sm">
          ...
          <div
            ...
            class="text-xs px-2 py-1 italic"
            style="color: var(--app-muted)"
          >
