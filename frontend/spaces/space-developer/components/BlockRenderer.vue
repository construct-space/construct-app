...
import { Loader2, Check, X, ChevronDown, ChevronUp, Brain, Wrench, ArrowRight, HelpCircle } from 'lucide-vue-next'
...
export type Block = ToolCallBlock | ThinkingBlock | TextBlock | PlanBlock | QuestionsBlock | HandoffBlock
...
// Render-only grouping of consecutive tool calls into one collapsible block.
...
function isGroupOpen(g: ToolGroupBlock): boolean {
  return groupOpen.value.has(g.id)
}
...
// Render-time unwrap of the pi-shape call_tool dispatcher → the real tool, for
// blocks streamed before the handler-level unwrap landed (or persisted).
...
// The one meaningful argument to show inline as `Tool(arg)`.
...
const groupedBlocks = computed<RenderBlock[]>(() => {
  ...
  for (const b of props.blocks) {
    ...
    // excluded — it renders as its own question card.
...
{{ askQuestionText(block.input) }}
      ...
      <!-- tool group — consecutive tool calls collapsed into one expandable,
           each rendered Claude-Code-style as `Tool(arg)`. -->
      <div
        ...
        <div v-if="isGroupOpen(block)" style="border-top: 1px solid var(--app-border)">
          ...
          >
            ...
            >
              ...
              <ChevronUp v-if="toolOpen.has(t.callId)" :size="10" class="shrink-0" style="color: var(--app-muted)" />
              <ChevronDown v-else :size="10" class="shrink-0" style="color: var(--app-muted)" />
            </button>
            <div v-if="toolOpen.has(t.callId)" class="px-2.5 pb-2 pt-0 space-y-1.5">
              <div
                v-if="t.input"
