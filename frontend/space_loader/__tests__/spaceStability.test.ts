...
describe('HOST_PROVIDED_PACKAGES', () => {
  ...
  it('does not expose native tauri packages to dynamic spaces', () => {
    expect(HOST_PROVIDED_PACKAGES).not.toContain('@tauri-apps/api')
    expect(HOST_PROVIDED_PACKAGES).not.toContain('@tauri-apps/api/core')
    expect(HOST_PROVIDED_PACKAGES).not.toContain('@tauri-apps/plugin-fs')
    expect(HOST_PROVIDED_PACKAGES).not.toContain('@tauri-apps/plugin-shell')
  })
  ...
  it('includes construct packages', () => {
    ...
    expect(HOST_PROVIDED_PACKAGES).toContain('@construct-space/sdk')
    expect(HOST_PROVIDED_PACKAGES).not.toContain('@construct/sdk')
  })
...
// ---------------------------------------------------------------------------
// Dynamic CSS isolation
// ---------------------------------------------------------------------------

describe('dynamic space CSS isolation', () => {
  it('prefixes space selectors before injecting them into the host document', () => {
    const scoped = scopeSpaceCssForHost(
      'pinball',
      '.grid button{height:60px;font-size:22px}.card h2,.link:hover{color:red}',
    )

    expect(scoped).toContain('[data-construct-space="pinball"] .grid button{height:60px;font-size:22px}')
    expect(scoped).toContain('[data-construct-space="pinball"] .card h2,[data-construct-space="pinball"] .link:hover{color:red}')
    expect(scoped).not.toMatch(/(^|[,{])\.grid button/)
  })

  it('scopes rules inside group at-rules but leaves keyframe steps untouched', () => {
    const scoped = scopeSpaceCssForHost(
      'pinball',
      '@layer utilities{.grid button{height:60px}}@media (max-width: 700px){body .panel{display:none}}@keyframes spin{to{transform:rotate(360deg)}}',
    )

    expect(scoped).toContain('@layer utilities{[data-construct-space="pinball"] .grid button{height:60px}}')
    expect(scoped).toContain('@media (max-width: 700px){[data-construct-space="pinball"] .panel{display:none}}')
    expect(scoped).toContain('@keyframes spin{to{transform:rotate(360deg)}}')
    expect(scoped).not.toContain('[data-construct-space="pinball"] to{')
  })

  it('preserves blockless at-rules before scoped selectors', () => {
    const scoped = scopeSpaceCssForHost(
      'pinball',
      '@layer theme, base, components, utilities;.grid button{height:60px}',
    )

    expect(scoped).toBe('@layer theme, base, components, utilities;[data-construct-space="pinball"] .grid button{height:60px}')
  })

  it('keeps Vue scoped fallback local to the space root', () => {
    const fallback = buildVueScopedFallback('.grid[data-v-68f0a7c0] button[data-v-68f0a7c0]{font-size:22px}')
    const scoped = scopeSpaceCssForHost('pinball', fallback)

    expect(fallback).toContain('.grid button{font-size:22px}')
    expect(scoped).toContain('[data-construct-space="pinball"] .grid button{font-size:22px}')
  })
})
...
describe('validateSpaceManifest', () => {
  ...
  it('rejects a manifest missing scopes', () => {
    const result = validateSpaceManifest({
      id: 'x',
      name: 'X',
      version: '0.1.0',
    ...
    expect(result.valid).toBe(false)
    expect(result.errors.some(e => /scopes/.test(e))).toBe(true)
  })

  it('accepts scopes:["app"]', () => {
    const r = validateSpaceManifest(validManifest({ scopes: ['app'] }))
    expect(r.errors).toEqual([])
  })

  it('accepts scopes:["app","org"]', () => {
    const r = validateSpaceManifest(validManifest({ scopes: ['app', 'org'] }))
    expect(r.errors).toEqual([])
  })

  it('rejects scopes with unknown value', () => {
    const r = validateSpaceManifest(validManifest({ scopes: ['bogus'] as unknown as string[] }))
    expect(r.errors.some(e => /scopes/.test(e))).toBe(true)
  })

  it('rejects the legacy single "scope" field', () => {
    const r = validateSpaceManifest(validManifest({ scope: 'app' as unknown as 'app' }))
    expect(r.valid).toBe(false)
    expect(r.errors.some(e => e.includes('"scope" is no longer supported'))).toBe(true)
  })

  it('rejects projectAware that is not a boolean', () => {
    const r = validateSpaceManifest(validManifest({ projectAware: 'yes' as unknown as boolean }))
    expect(r.errors.some(e => /projectAware/.test(e))).toBe(true)
  })
})
...
describe('normalizeManifestScopes', () => {
  it('passes through new scopes:[] shape', () => {
    expect(normalizeManifestScopes({ scopes: ['app'] })).toEqual({ scopes: ['app'], projectAware: false })
    expect(normalizeManifestScopes({ scopes: ['org'], projectAware: true })).toEqual({ scopes: ['org'], projectAware: true })
    expect(normalizeManifestScopes({ scopes: ['app', 'org'] })).toEqual({ scopes: ['app', 'org'], projectAware: false })
  })

  it('falls back to scopes:["app"] for missing or invalid scopes', () => {
    expect(normalizeManifestScopes({})).toEqual({ scopes: ['app'], projectAware: false })
    expect(normalizeManifestScopes({ scopes: ['bogus'] })).toEqual({ scopes: ['app'], projectAware: false })
  })
})
