 ...
 *
 * Host-native spaces (architect, ask, coder, project) have their own
...
import { loadSpace, reloadSpace, watchSpace, getLastLoadError, clearLastLoadError, type LoadedSpace, type SpaceLoadError } from '@/space_loader/SpaceLoader'
...
import { matchRoutePattern } from '@/space_loader/routeMatching'
import { Loader2, AlertCircle, RefreshCw, ShoppingBag, Wrench } from 'lucide-vue-next'
import { shallowRef, markRaw, onErrorCaptured } from 'vue'
...
// Tracks whether this instance is still mounted. Async work (load,
// setupDevWatcher, dev-watch callbacks, telemetry imports) must check this
// before mutating refs — otherwise mutations on an unmounted instance
// schedule update jobs against a dead vnode and Vue throws
// "null is not an object (component.emitsOptions / instance.type)".
let alive = true

/** Actionable suggestions derived from the error message */
const _errorActions = computed<ErrorAction[]>(() => {
...
const currentPage = computed(() => {
  ...
  // 2. Pattern match (parameterized pages like "members/:id" or "members/[id]")
  for (const key of Object.keys(space.value.pages)) {
    if (!key.includes(':') && !key.includes('[')) continue
    const params = matchRoutePattern(path, key)
    if (params) {
...
function updateSidebarForSpace(loaded: LoadedSpace) {
  ...
  const visiblePages = pages.filter(p => !(p as unknown as Record<string, unknown>).hidden)
  ...
  const pageItems: SpaceNavItem[] = visiblePages.map(p => ({
    ...
    requiresContext: (p as unknown as Record<string, unknown>).requiresContext as boolean | undefined,
  }))
  enterSpace(props.spaceName, pageItems, '/app')
}

/** Load the space on mount and when spaceName changes */
async function load(options: { forceReload?: boolean } = {}) {
  loading.value = true
  error.value = null
  try {
    ...
    const loaded = options.forceReload
      ? await reloadSpace(props.spaceName)
      : await loadSpace(props.spaceName)
    if (!alive) return
    const loadMs = performance.now() - loadStart
    applyLoaded(loaded)

    // Telemetry: track space load time
    if (loaded) {
      import('@/composables/useTelemetry').then(({ useTelemetry }) => {
        if (!alive) return
        useTelemetry().trackPerf('space.load', loadMs)
      }).catch(() => { })
    }
    if (loaded && !props.projectId) {
...
async function setupDevWatcher() {
  ...
  unwatchFn?.()
  unwatchFn = null
  if (isCoreSpace(props.spaceName)) return
  const { loadSpaces } = useSpaces()
  // watchSpace returns null if no dev session is active for this space
  const fn = await watchSpace(props.spaceName, async (reloaded) => {
    if (!alive) return
  ...
  if (!alive) {
    // Component unmounted while we were awaiting watchSpace — drop the
    // subscription instead of leaking a callback that will mutate dead refs.
    fn?.()
    return
  }
  unwatchFn = fn
...
onErrorCaptured((err) => {
  error.value = `Space error: ${err instanceof Error ? err.message : String(err)}`
  loadError.value = null
  loading.value = false
  console.error('[DynamicSpacePage] Space render error:', err)
  return false
})
...
<template>
  <div class="h-full flex flex-col" :data-construct-space="spaceName">
    <!-- Loading state -->
    ...
    <div v-else-if="error" class="flex-1 flex items-center justify-center p-6">
      <div class="max-w-md w-full">
        ...
        <div v-if="loadError"
          class="mb-6 rounded-lg border border-[var(--app-border)] bg-[var(--app-background)] overflow-hidden">
          <!-- Error phase badge -->
          ...
          <div v-if="loadError.details?.length" class="px-4 py-3 space-y-1">
            <p v-for="(detail, i) in loadError.details" :key="i" class="text-xs text-[var(--app-muted)] font-mono">
              {{ detail }}
            </p>
          </div>
          ...
          <div
            class="px-4 py-3 border-t border-[var(--app-border)] bg-[color-mix(in_srgb,var(--app-muted)_3%,transparent)]">
            <p class="text-xs font-medium text-[var(--app-muted)] mb-2">Suggested actions:</p>
            <ul class="space-y-1">
              <li v-for="(action, i) in suggestedActions(loadError.phase)" :key="i"
    ...
    <template v-else-if="space">
      ...
      <component v-if="currentPage" :is="currentPage" :key="`${spaceName}-${currentPagePath}`" :project-id="projectId"
        v-bind="routeParams" />
      ...
      <div v-else class="flex-1 flex items-center justify-center">
        <div class="text-center max-w-sm">
          <div class="size-16 rounded-2xl flex items-center justify-center mx-auto mb-4" :class="theme.bg">
