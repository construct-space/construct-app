...
export function validateSpaceManifest(manifest: Record<string, unknown>): ManifestValidationResult {
  ...
  // Required: scopes array. projectAware is the orthogonal flag for
  // project-context behaviour and must be boolean when present.
  if (manifest.scope !== undefined) {
    errors.push('"scope" is no longer supported — use scopes: ["app"|"org"] + projectAware: boolean')
  }
  if (!Array.isArray(manifest.scopes) || manifest.scopes.length === 0 || !manifest.scopes.every(s => s === 'app' || s === 'org')) {
    errors.push('Missing or invalid "scopes" — must be a non-empty array of "app" | "org"')
  }
  if (manifest.projectAware !== undefined && typeof manifest.projectAware !== 'boolean') {
    errors.push('"projectAware" must be a boolean')
  }
  if (errors.length === 0) {
    ;(manifest as { projectAware: boolean }).projectAware = manifest.projectAware === true
  }
...
const FORBIDDEN_NATIVE_PATTERNS: Array<[RegExp, string]> = [
  [/@tauri-apps\/api/g, '@tauri-apps/api'],
  [/@tauri-apps\/plugin-/g, '@tauri-apps/plugin-*'],
  [/window\.__TAURI__/g, 'window.__TAURI__'],
  [/__TAURI_INTERNALS__/g, '__TAURI_INTERNALS__'],
  [/window\.__CONSTRUCT__\[['"]@tauri-apps\//g, 'window.__CONSTRUCT__[@tauri-apps/*]'],
]

/**
 * Release-safety gate for dynamic marketplace bundles.
 *
 * Dynamic spaces run inside the app webview today, so native bridge access
 * must not be part of the public space contract. This is still best-effort
 * static screening, but it blocks the accidental and generated cases that
 * previously externalized Tauri modules through window.__CONSTRUCT__.
 */
export function scanBundleForForbiddenNativeAccess(bundleContent: string): string[] {
  const matches: string[] = []
  for (const [pattern, label] of FORBIDDEN_NATIVE_PATTERNS) {
    pattern.lastIndex = 0
    if (pattern.test(bundleContent)) matches.push(label)
  }
...
export interface SpaceLoadError {
  spaceId: string
  phase: 'manifest' | 'validation' | 'version' | 'checksum' | 'signature' | 'bundle' | 'eval' | 'export'
  message: string
  details?: string[]
}
