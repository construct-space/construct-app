...
/** Remove a space's request handler, usually because its runtime was unloaded. */
export function unregisterContextHandler(spaceId: string): void {
  handlers.delete(spaceId)
}
...
/** Remove a space's automation provider, usually because its runtime was unloaded. */
export function unregisterAutomationProvider(spaceId: string): void {
  automationProviders.delete(spaceId)
}
...
/** Clear cached context and registered callbacks owned by an unloaded space. */
export function clearSpaceRuntimeState(spaceId: string): void {
  unregisterContextHandler(spaceId)
  unregisterAutomationProvider(spaceId)

  for (const [type, payload] of latestContext) {
    if (payload.spaceId === spaceId) {
      latestContext.delete(type)
    }
  }
