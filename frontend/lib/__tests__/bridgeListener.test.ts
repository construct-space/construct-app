...
vi.mock('@tauri-apps/api/webviewWindow', () => ({
  getCurrentWebviewWindow: () => ({
    listen: vi.fn(async (_event: string, handler: (event: unknown) => void) => {
      ...
      ;(globalThis as Record<string, unknown>).__bridgeHandler = handler
      return () => {} // unlisten
    }),
  }),
}))
...
vi.mock('@tauri-apps/api/core', () => {
  ...
  return {
    invoke: vi.fn(async (_cmd: string, args: { id: string; result: unknown }) => {
      responses.push(args)
    }),
...
async function simulateBridgeRequest(id: string, method: string, params?: Record<string, unknown>) {
  const handler = (globalThis as Record<string, unknown>).__bridgeHandler as ((event: unknown) => Promise<void>) | undefined
  ...
  const mod = await import('@tauri-apps/api/core') as unknown as { __getResponses: () => Array<{ id: string; result: Record<string, unknown> }> }
  const responses = mod.__getResponses()
...
describe('bridgeListener', () => {
  ...
  beforeEach(async () => {
    // Clear state
    const { __clearResponses } = await import('@tauri-apps/api/core') as unknown as { __clearResponses: () => void }
    __clearResponses()
    stopBridgeListener()
    setActiveSpace(null)
    if (unregProvider) {
  ...
  it('starts the listener and registers handler', async () => {
    ...
    expect((globalThis as Record<string, unknown>).__bridgeHandler).toBeDefined()
  })
