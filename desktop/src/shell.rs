...
fn validate_shell_command(command: &str, args: &[String], cwd: &str) -> Result<(), String> {
    ...
    if command.is_empty()
        ...
        || command.contains('\0')
    {
        return Err("Command is not allowed".to_string());
    }

    const ALLOWED_COMMANDS: &[&str] = &[
        "bun",
        "bunx", // `bun x` — npx-equivalent. Used by editor run targets.
        "cargo",
        "composer",
        "construct",
        "curl",
        ...
        "elixir",
        "flutter",
        "gh", // GitHub CLI — read-only org/auth queries from OrgProjectDetailPage
        "git",
        "go",
        "kill",
        "lsof",
        ...
        "rails",
        "ruby",
        "rustc", // Rust compiler version probe in DeveloperSettings runtime detect.
        "stat",
        "tar",
        "which",
        "xcodebuild",
    ...
    if cwd.is_empty() || cwd.contains('\0') || !Path::new(cwd).is_absolute() {
        return Err("Working directory is not allowed".to_string());
    }
    ...
    {
        return Err("Command argument is not allowed".to_string());
    }

    if command == "pkill" && (args.len() != 2 || args[0] != "-f" || args[1] != "flutter_tools") {
        return Err("pkill is restricted to Flutter cleanup".to_string());
    }

    // NOTE: the allowlist above is the gate. It deliberately includes
    // interpreters (node, python3, go, bun, …) and curl, so a caller that
    // can reach this command can still run arbitrary code (`node -e`,
    // `curl file://…`). This is acceptable only because reaching
    // run_shell_command requires the privileged app webview; it is NOT a
    // containment boundary for untrusted (space) code. See the space
    // trust-model note in the review.
...
pub async fn run_shell_command(
    ...
    validate_shell_command(&command, &args, &cwd)?;
