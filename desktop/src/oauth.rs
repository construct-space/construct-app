...
fn generate_pkce() -> Result<(String, String), String> {
    ...
    // Propagate failure instead of falling back to all-zero bytes — a
    // predictable verifier defeats PKCE entirely.
    getrandom::getrandom(&mut verifier_bytes)
        .map_err(|e| format!("CSPRNG unavailable for PKCE verifier: {}", e))?;
...
fn generate_state() -> Result<String, String> {
    ...
    // A zero state would be trivially guessable, defeating CSRF protection.
    getrandom::getrandom(&mut state_bytes)
        .map_err(|e| format!("CSPRNG unavailable for OAuth state: {}", e))?;
...
pub async fn oauth_exchange(
    ...
    // CSRF defence: the state returned via the callback (either embedded as
    // `code#state` or passed separately) must match the one we issued in
    // oauth_start. A non-empty mismatch means the code didn't originate from
    // our flow — abort rather than exchange it. When no state is returned at
    // all we fall back to PKCE alone (the verifier still binds the code).
    let returned_state = code_state
        .clone()
        .filter(|s| !s.is_empty())
        .or_else(|| if state.is_empty() { None } else { Some(state.clone()) });
    if let Some(s) = returned_state {
        if s != pending.state {
            // Clear the pending flow so a stale/forged attempt can't be retried.
            if let Ok(mut oauth) = oauth_state.lock() {
    ...
    let state_assignment = if let Some(ref s) = code_state {
        format!(
            "body.state = {};",
            serde_json::to_string(s).map_err(|e| format!("Failed to encode OAuth state: {}", e))?
        )
    } else {
        String::new()
    };
    ...
    let exchange_js = format!(
        ...
        "#,
        serde_json::to_string(&actual_code)
            .map_err(|e| format!("Failed to encode OAuth code: {}", e))?,
        serde_json::to_string(&pending.client_id)
            .map_err(|e| format!("Failed to encode OAuth client id: {}", e))?,
        serde_json::to_string(&pending.redirect_uri)
            .map_err(|e| format!("Failed to encode OAuth redirect uri: {}", e))?,
        serde_json::to_string(&pending.code_verifier)
            .map_err(|e| format!("Failed to encode OAuth verifier: {}", e))?,
        state_assignment
    );
    ...
    for _ in 0..30 {
        ...
        if let Ok(title) = exchange_window.title() {
            if title.starts_with("OAUTH_DONE:") {
                ...
                // Do NOT log json_str — it carries the access + refresh
                // tokens, and stderr is persisted to the on-disk log file.
                eprintln!("[OAuth] Token exchange completed (webview fallback)");
...
pub async fn construct_auth_exchange_code(
    ...
    // OAuth paths on the gateway live under /api/oauth/* (proxied to
    // accounts' /oauth/* internally). Hitting /oauth/token directly on
    // my.construct.space falls through to the SPA location and gets 405.
    let token_url = format!("{}/api/oauth/token", normalize_api_base(&api_base));
    let client = reqwest::Client::builder()
...
pub async fn construct_auth_profile(
    ...
    // Accounts' /api/me lives at /api/accounts/me on the gateway.
    let profile_url = format!("{}/api/accounts/me", normalize_api_base(&api_base));
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(20))
        .build()
        .map_err(|e| format!("Failed to create HTTP client: {}", e))?;
