...
# Load Tauri updater signing key (shared with construct-releases)
# shellcheck source=load-signing-key.sh
source "$SCRIPT_DIR/load-signing-key.sh"

# 1. Build brain sidecar (renamed from operator in the 1.1 refactor)
echo "==> Building brain..."
bash "$SCRIPT_DIR/build-brain.sh"
